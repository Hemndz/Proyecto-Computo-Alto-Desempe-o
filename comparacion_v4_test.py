...
MODELOS = {
    "v2-nano (baseline)": BASE / "runs" / "dron_pose_v2_nano" / "weights" / "best.pt",
    "v4 (domain rand. bbox)": BASE / "runs" / "dron_pose_v4" / "weights" / "best.pt",
}
...
def main():
    ...
    for nombre, pesos in MODELOS.items():
        if not pesos.exists():
            print(f"[SKIP] {nombre}: no existe {pesos}\n")
            continue
        print(f"========== {nombre} ==========")
